...
(function () {
  ...
  // Strips a phone number down to digits only before it goes into a
  // wa.me link. Numbers typed/saved from the Telegram admin bot are
  // often formatted like "+91 6001877518" or "091-6001-877518" — very
  // natural for a human to type, but wa.me only accepts digits (no +,
  // spaces or dashes). An unsanitized number in the URL makes WhatsApp
  // serve its own generic 404 ("This page doesn't exist") instead of
  // opening a chat, for every wa.me link on the site (Submit, Message
  // Your Guide, Send via WhatsApp instead, refund chat, nav menu —
  // all of them use CONTENT.whatsappNumber or a guide's saved number).
  function waNumber(n) { return String(n || "").replace(/\D/g, ""); }

  // 📵 Manual WhatsApp Mode always opens THIS number — deliberately
  // separate from CONTENT.whatsappNumber (which every other WhatsApp
  // link on the site still uses: refund chat, "Send via WhatsApp
  // instead", nav menu, etc.). Editable here only, not from the
  // Telegram bot's content editor — change the digits below if this
  // number ever changes.
  var MANUAL_MODE_WHATSAPP_NUMBER = "918787679579";
  ...
  // A small photo strip. Give it a list of image file names via the
  // "images" prop (e.g. from a highlight's images: [...] list in
  // config.js) and it slides to the next one every few seconds, plus
  // supports a left/right touch swipe. Shows nothing if the list is
  // empty; shows a single still photo (no arrows/dots/sliding) if
  // there's only one.
  ...
  // ImageSlot / MediaSlot — one optional media spot placed anywhere.
  // Renders nothing (no gap, page looks completely normal) when
  // disabled or empty. Accepts a slot shaped { enabled, type, image,
  // images, video, alt } — "type" auto-detects from which fields are
  // filled in: "video" (single clip, tap to play), "gallery" (2+
  // images.length, swipe/arrows/dots), or "image" (single photo).
  function detectSlotType(slot) {
    if (slot.type) return slot.type;
    if (slot.video) return "video";
    if (Array.isArray(slot.images) && slot.images.length > 1) return "gallery";
    return "image";
  }

  function MediaSlotVideo(props) {
    return h(
      "div", { className: props.className || "mt-4 rounded-xl overflow-hidden aspect-[16/9] bg-black/20" },
      h(
        "video",
        { controls: true, playsInline: true, poster: props.poster || undefined, className: "w-full h-full object-cover" },
        h("source", { src: props.src, type: "video/mp4" })
      )
    );
  }
  ...
  function ImageSlot(props) {
    ...
    if (type === "video") {
      ...
      return h(MediaSlotVideo, { src: slot.video, poster: slot.image, className: props.className });
    }
    if (type === "gallery") {
  ...
  function App() {
    ...
    // home page's "Booking Form" link jumps straight to ?page=3, skipping
    // the Packages page (2) where pkg would normally get set by tapping a
    // package card. Without this, pkg stays null on page 3/4, so the
    // sharedTour-only booking fields never render and the invoice total
    // is stuck at ₹0. Auto-select the only package there is.
    var pkgState = useState(SECTIONS.privatePackageCard ? null : "sharedTour");
    var pkg = pkgState[0], setPkg = pkgState[1];
    ...
    // "Know Before You Go" starts collapsed; visitors tap the header
    // to expand it.
    var guideOpenState = useState(false); var visitorGuideOpen = guideOpenState[0], setVisitorGuideOpen = guideOpenState[1];
    ...
    // 🎁 Referral/discount code — see krem-chympe/app.js for the same
    // pattern; kept identical across both sites on purpose.
    var refCodeState = useState(""); var referralCode = refCodeState[0], setReferralCode = refCodeState[1];
    var refAppliedState = useState(null); var referralApplied = refAppliedState[0], setReferralApplied = refAppliedState[1];
    ...
    function clearDraft() {
      // This site sells exactly one package when privatePackageCard is
      // off (see pkgState's initializer above) — a reset should land
      // back on that same default, not null, or the booking fields
      // would go blank instead of re-selecting the only package there is.
      setPkg(SECTIONS.privatePackageCard ? null : "sharedTour");
    ...
    function submitBookingViaWhatsApp() {
      ...
      window.KCBridge.submitBooking({
        ...
        if (res && res.ok && res.bookingId) {
          ...
          // 📵 Manual WhatsApp Mode — the backend never posted this to
          // Telegram (see manual-mode.js); it's just waiting for the
          // admin to paste this bookingId back into the bot. There's no
          // live confirmation to poll for, so — deliberately, per how
          // this mode works — no watchStatus() call here at all. The
          // visitor's WhatsApp opens immediately with everything
          // prefilled, the same code shown on-page for their reference.
          if (res.manualMode) {
            setTrackingId(res.bookingId);
            setBookingStatus("manual");
            setReceiptLocked(false);
            setPage(6);
            var manualMsg = buildBookingMessageText() + "\n\n🔑 Code: " + res.bookingId;
            window.open("https://wa.me/" + MANUAL_MODE_WHATSAPP_NUMBER + "?text=" + encodeURIComponent(manualMsg), "_blank");
            return;
          }
    ...
    function invoiceLines() {
      if (pkg === "sharedTour") {
        ...
        if (totals.salePercent > 0) {
          ...
          stLines.push(["Price Per Person", money(totals.pricePerPerson)]);
        }
    ...
    function whatsappLink() {
      if (bookingStatus === "confirmed" && guideContact.phone) {
        return "https://wa.me/" + waNumber(guideContact.phone) + "?text=" + encodeURIComponent(confirmedGuideMessageText());
      }
      return "https://wa.me/" + waNumber(CONTENT.whatsappNumber) + "?text=" + encodeURIComponent(buildBookingMessageText());
    }
    ...
    function navigateItem(item) {
      if (item && typeof item === "object") {
        ...
        if (item.whatsapp) { setMobileMenuOpen(false); window.open("https://wa.me/" + waNumber(CONTENT.whatsappNumber), "_blank"); return; }
      }
    ...
    var noResponseCard = h(
      ...
      h("h2", { className: "mt-6 text-3xl font-bold" }, "Still Waiting On Your Guide"),
      h(
        "div",
        ...
        "No response from your guide yet"
      ),
      h(
        "p", { className: "mt-4 text-white/60 text-sm leading-relaxed" },
        "Your guide hasn't confirmed or rejected this booking yet — they may be mid-tour or away from their phone. Tap below to chat with our admin directly on WhatsApp and we'll sort it out right away."
      ),
      trackingId && h(
      ...
      h(
        ...
        "Give our admin this code on WhatsApp — it lets them pull up your booking instantly."
      ),
      h(
        "div", { className: "mt-6 flex flex-col gap-3" },
        h(
          "a", { href: noResponseWhatsappLink(), target: "_blank", rel: "noopener", className: "kc-whatsapp-btn ring-2 ring-amber-400/60" },
          h(Phone, { size: 18 }), "Chat With Admin on WhatsApp"
        )
      ),
    ...
    var normalStatusCard = h(
      ...
      bookingStatus === "pending" && h(CountdownDisplay, {
        ...
        label: "Checking with your guide… ",
      ...
      bookingStatus === "confirmed" && guideContact.name && h(
        "p", { className: "mt-1 text-white/40 text-[11px]" }, "Confirmed by ", guideContact.name
      ),
      ...
      bookingStatus === "cancelled" && h(
        "p", { className: "mt-4 text-[12px] text-amber-300" }, "Tap below to chat with our admin directly on WhatsApp — give them the code above and we'll sort it out right away."
      ),
      h(
        "div", { className: "mt-6 flex flex-col gap-3" },
        h(
          ...
          h(Phone, { size: 18 }),
          bookingStatus === "confirmed" ? "Message Your Guide" : bookingStatus === "cancelled" ? "Chat With Admin on WhatsApp" : "WhatsApp"
        )
      ),
        ...
        bookingStatus === "confirmed" && h(
          ...
          refundStatus === "requested" && h(
            ...
            "Refund request sent — we will notify you once it's reviewed."
          ),
          ...
          refundError && h(
            ...
            h("a", { href: refundWhatsappLink(), target: "_blank", rel: "noopener", className: "text-xs underline text-white/70 hover:text-white" }, "Or message us directly on WhatsApp")
          )
        ),
      (bookingStatus === "pending" && isBookingLocked())
        ? h("div", { className: "mt-4 text-[12px] text-white/50 leading-relaxed text-center" }, "Since you've already sent your payment receipt, we're keeping this booking open until your guide or our admin confirms or rejects it — use WhatsApp above if you need anything in the meantime.")
    ...
    // 📵 Manual WhatsApp Mode confirmation screen — shown instead of the
    // normal live-tracking card when bookingStatus === "manual" (see
    // submitBookingViaWhatsApp above). Deliberately has no countdown, no
    // "waiting for guide" spinner, and no lock on leaving the page — per
    // how this mode works, there is nothing here to poll for until the
    // admin pastes the code into the bot.
    var manualModeCard = h(
      GlassCard, { className: "p-10 text-center" },
      h("div", { className: "w-20 h-20 mx-auto rounded-full border flex items-center justify-center bg-emerald-500/20 border-emerald-400/30" }, h(Phone, { size: 36, className: "text-emerald-300" })),
      h("h2", { className: "mt-6 text-3xl font-bold" }, "Booking Sent — Check WhatsApp"),
      h(
        "div",
        { className: "mt-3 inline-flex items-center gap-2 px-4 py-2 rounded-full text-xs border bg-emerald-500/15 border-emerald-400/30 text-emerald-300" },
        h("span", { className: "w-2 h-2 rounded-full bg-current" }),
        "We opened WhatsApp with your details prefilled"
      ),
      h(
        "p", { className: "mt-4 text-white/60 text-sm leading-relaxed" },
        "Send that WhatsApp message to us and our team will confirm your booking directly there — just keep the chat open."
      ),
      trackingId && h(
        "div", { className: "mt-4 inline-block px-4 py-2 rounded-full bg-emerald-500/10 border border-emerald-400/30 text-emerald-200 text-sm font-mono" },
        "Booking Code: ", trackingId
      ),
      h(
        "div", { className: "mt-6 flex flex-col gap-3" },
        h(
          "a", {
            href: "https://wa.me/" + MANUAL_MODE_WHATSAPP_NUMBER + "?text=" + encodeURIComponent(buildBookingMessageText() + "\n\n🔑 Code: " + (trackingId || "")),
            target: "_blank", rel: "noopener", className: "kc-whatsapp-btn"
          },
          h(Phone, { size: 18 }), "Open WhatsApp Again"
        )
      ),
      h(
        "button",
        { onClick: function () { setBookingCode(""); setTrackingId(""); setBookingStatus("pending"); setSubmitError(""); setSubmitted(false); setPage(1); }, className: "mt-4 w-full bg-white/5 border border-white/10 py-3 rounded-full font-semibold" },
        "Back to Home"
      )
    );
    ...
    var page6 = page === 6 && h(
      ...
      bookingStatus === "manual" ? manualModeCard : (noResponse && bookingStatus === "pending") ? noResponseCard : normalStatusCard
    );
