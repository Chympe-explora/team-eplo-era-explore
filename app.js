...
(function () {
  ...
  // often formatted like "+91 6001877518" — very natural for a human
  // to type, but wa.me only accepts digits (no +, spaces or dashes).
  // An unsanitized number makes WhatsApp serve its own generic 404
  // ("This page doesn't exist") instead of opening a chat.
  ...
  // the page. Reads window.KC_CONTENT.imageSlots[key] = { enabled,
  // type, image, images, video, alt }. Renders nothing (no gap, page
  // looks completely normal) whenever the slot is turned off or has no
  // media yet — so these are safe to sprinkle anywhere. Turn a slot
  // on/off and upload its photo(s)/video from Telegram Admin →
  // 🖼️ Change Photos / 🎬 Slot Videos.
  //
  // Slot "type" (auto-detected if not set):
  //   "image"   — a single still photo (default)
  //   "gallery" — 2+ photos in slot.images[]; swipeable on touch, with
  //               arrows/dots on desktop, and a gentle auto-advance
  ...
  function MediaGallerySwipe(props) {
    var images = (props.images || []).filter(Boolean);
    var className = props.className || "mt-1 rounded-xl overflow-hidden aspect-[16/9] bg-black/20";
    ...
    if (images.length === 1) {
      return h(
        ...
        h("img", { src: images[0], alt: props.alt || "", loading: "lazy", decoding: "async", className: "w-full h-full object-cover" })
      );
    }

    function goTo(i) { setIdx((i + images.length) % images.length); }
    function onTouchStart(e) {
      dragState.current = { startX: e.touches[0].clientX, dx: 0, dragging: true };
    }
    function onTouchMove(e) {
      if (!dragState.current.dragging) return;
      dragState.current.dx = e.touches[0].clientX - dragState.current.startX;
    }
    function onTouchEnd() {
    ...
    return h(
      ...
      h(
        "button",
        { type: "button", onClick: function () { goTo(safeIdx - 1); }, className: "hidden md:flex absolute left-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/50 hover:bg-black/70 items-center justify-center text-white", "aria-label": "Previous photo" },
        h(ArrowRight, { size: 14, className: "rotate-180" })
      ),
      h(
        "button",
        { type: "button", onClick: function () { goTo(safeIdx + 1); }, className: "hidden md:flex absolute right-2 top-1/2 -translate-y-1/2 w-7 h-7 rounded-full bg-black/50 hover:bg-black/70 items-center justify-center text-white", "aria-label": "Next photo" },
  ...
  function App() {
    ...
    // closes whichever was open. destMenuPos holds the tapped button's
    // on-screen position so the popover can be positioned with `fixed`
    // — the card it lives in has overflow-hidden (to clip the photo's
    // rounded corners), which would otherwise clip an `absolute`
    // popover too, since overflow-hidden clips ANY positioned
    // descendant regardless of z-index. `fixed` is computed relative to
    // the viewport, not any ancestor, so it escapes that clipping.
    var destMenuState = useState(null); var openDestMenu = destMenuState[0], setOpenDestMenu = destMenuState[1];
    var destMenuPosState = useState(null); var destMenuPos = destMenuPosState[0], setDestMenuPos = destMenuPosState[1];

    function toggleDestMenu(id, buttonEl) {
      if (openDestMenu === id) { setOpenDestMenu(null); return; }
      var rect = buttonEl.getBoundingClientRect();
      setDestMenuPos({ top: rect.top, left: rect.left, width: rect.width });
      setOpenDestMenu(id);
    }

    // The popover's position is computed once, at tap time, from the
    // button's on-screen coordinates — closing it on scroll (rather
    // than trying to keep repositioning it) is simpler and avoids any
    // drift between the popover and the button that opened it.
    useEffect(function () {
      if (!openDestMenu) return;
      function closeOnScroll() { setOpenDestMenu(null); }
      window.addEventListener("scroll", closeOnScroll, { passive: true });
      return function () { window.removeEventListener("scroll", closeOnScroll); };
    }, [openDestMenu]);
...
function closeNotice() {
  setShowNotice(false);
    ...
    function navigateTo(item) {
      ...
      if (item.whatsapp) {
        setMobileMenuOpen(false);
        window.open("https://wa.me/" + waNumber(CONTENT.whatsappNumber), "_blank");
        return;
      }
      if (item.tel) {
        setMobileMenuOpen(false);
    ...
    var destinations = h(
      ...
      // Rendered as a page-level overlay (position: fixed, computed
      // from the tapped button's on-screen position) rather than nested
      // inside the destination card — that card has overflow-hidden
