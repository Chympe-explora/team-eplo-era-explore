...
(function () {
  ...
  // ImageSlot / MediaSlot — one optional media spot. Renders nothing
  // (no gap, page looks completely normal) when disabled or empty, so
  // it's safe to place anywhere. Toggle on/off, upload photo(s), or
  // upload a video from Telegram Admin → 🖼️ Change Photos / 🎬 Slot
  // Videos. Accepts a slot shaped { enabled, type, image, images,
  // video, alt } — "type" is auto-detected from which fields are
  // filled in when not set explicitly:
  //   "image"   — a single still photo (default)
  //   "gallery" — 2+ photos in slot.images[]; swipe on touch, arrows/
  //               dots on desktop, gentle auto-advance
  //   "video"   — a single slot.video clip, tap to play with sound
  ...
  function App() {
    ...
    var header = h(
      ...
      h(
        ...
        h(
          ...
          h(FlipButton, { idleLabel: t("bookNow", "Book Now"), activeLabel: activeLabelFor(t("bookNow", "Book Now")), onDone: function () { goBookNow(); }, className: "hidden md:block bg-[#2E8B57] hover:bg-[#257a4b] px-5 py-2 rounded-full text-sm font-medium transition" }),
      ...
      mobileMenuOpen && h(
        ...
        h(FlipButton, { idleLabel: t("bookNow", "Book Now"), activeLabel: activeLabelFor(t("bookNow", "Book Now")), onDone: function () { goBookNow(); setMobileMenuOpen(false); }, className: "w-full bg-[#2E8B57] py-3 rounded-full font-medium" }),
    ...
    var page1 = page === 1 && h(
      ...
      h(
        ...
        h(
          ...
          h("div", { className: "mt-8 flex gap-3" }, h("button", { onClick: function () { goBookNow(); }, className: "bg-[#2E8B57] hover:bg-[#257a4b] px-7 py-3 rounded-full text-sm font-semibold flex items-center gap-2" }, "Book Now"))
        ),
        h(
          ...
          h(FlipButton, { idleLabel: t("bookNow", "Book Now"), activeLabel: activeLabelFor(t("bookNow", "Book Now")), onDone: function () { goBookNow(); }, className: "mt-6 w-full bg-[#2E8B57] hover:bg-[#257a4b] py-3 rounded-full text-sm font-semibold" })
        )
      ),
      SECTIONS.trustBar && h(
    ...
    var manualModeCard = h(
      ...
      h("h2", { className: "mt-6 text-3xl font-bold" }, t("manualSentTitle", "Booking Sent — Check WhatsApp")),
      h(
        "div",
        ...
        h("span", { className: "w-2 h-2 rounded-full bg-current" }),
        t("manualSentBadgeText", "We opened WhatsApp with your details prefilled")
      ),
      h(
        "p", { className: "mt-4 text-white/60 text-sm leading-relaxed" },
        t("manualSentBodyText", "Send that WhatsApp message to us and our team will confirm your booking directly there — just keep the chat open.")
      ),
      trackingId && h(
      ...
      h(
        ...
        h(
          ...
          h(Phone, { size: 18 }), t("openWhatsappAgainLabel", "Open WhatsApp Again")
        )
      ),
      h(
        "button",
